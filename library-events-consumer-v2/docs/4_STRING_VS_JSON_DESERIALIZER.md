  ...
  kafka:
    consumer:
      ...
      properties:
        ...
        spring.json.type.mapping: com.learnkafka.domain.LibraryEventDTO:com.learnkafka.dto.LibraryEventDto
```
...
| `spring.json.type.mapping` | Mapping: when the producer's type header says `com.learnkafka.domain.LibraryEventDTO`, map it to the consumer's `com.learnkafka.dto.LibraryEventDto`. |
...
4. Type mapping (`com.learnkafka.domain.LibraryEventDTO` → `com.learnkafka.dto.LibraryEventDto`) cleanly decouples the producer's domain model from the consumer's DTO.
